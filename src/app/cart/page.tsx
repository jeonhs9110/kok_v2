...
function readLangFromCookie(): Lang {
  ...
  const m = document.cookie.match(/(?:^|;\s*)kokkok_lang=([^;]+)/);
  const v = m?.[1];
...
export default function CartPage() {
  // Cart lives at /cart (no [lang] segment). Read the user's chosen language
  // from the kokkok_lang cookie set by LanguagePicker so the cart matches the
  // rest of the site instead of always rendering in Korean.
  const [lang, setLang] = useState<Lang>('kr');
  useEffect(() => { setLang(readLangFromCookie()); }, []);

  return (
    <I18nProvider isKorea={true} lang={lang}>
      <CartProvider>
