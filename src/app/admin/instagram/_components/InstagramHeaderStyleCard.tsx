...
/**
 * @handle title-row style editor — font size + text color + optional
 * background plate. Live preview shows the operator's chosen style
 * against the section's actual background tint. Migration 34.
 * Extracted from /admin/instagram/page.tsx at 2026-06-21.
 */
...
export default function InstagramHeaderStyleCard({
  handle,
  fontSize,
  textColor,
  bgEnabled,
  ...
  return (
    <div className="bg-white rounded border border-[#e5e7eb] p-5 space-y-4">
      <div className="flex items-baseline justify-between">
        ...
        <p className="text-xs text-[#9ca3af]">기본은 18px · 짙은 회색 · 배경 없음.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          />
          <p className="text-[10px] text-[#9ca3af] mt-1">10–48 사이. 기본 18.</p>
        </div>
        <div>
          <label className="text-[11px] font-semibold text-[#6b7280] uppercase tracking-wider">
            글자 색상
      ...
      <div className="pt-3 border-t border-[#f3f4f6]">
        ...
        <div className="flex justify-center py-6 bg-[#fafbfc] rounded">
          <div
            className="flex items-center gap-2 font-bold tracking-wide"
            style={{
              color: textColor,
              fontSize: `${Math.max(10, Math.min(48, parseInt(fontSize, 10) || 18))}px`,
              backgroundColor: bgEnabled ? bgColor : undefined,
              padding: bgEnabled ? '0.5rem 1rem' : undefined,
              borderRadius: bgEnabled ? '0.375rem' : undefined,
            }}
          >
