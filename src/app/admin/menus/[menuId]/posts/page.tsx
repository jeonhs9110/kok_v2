...
export default function PostsAdminPage() {
  ...
  return (
    <div className="space-y-6">
      <Link href="/admin/menus" className="inline-flex items-center gap-1 text-[12px] font-semibold text-[#6b7280] hover:text-black transition-colors">
        <ArrowLeft className="w-4 h-4" /> 메뉴 목록으로
      </Link>
      <div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) resetForm(); }}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 border-t border-[#e5e7eb] bg-[#fafbfc] flex justify-end gap-2">
              <button onClick={resetForm} className="px-4 py-2.5 border border-[#d1d5db] text-[#374151] rounded text-sm font-semibold bg-white hover:bg-[#f9fafb] transition-colors">취소</button>
